...
astrolist="$(echo "$astrolist" | sed 's^ ^ astro/^g' | sed 's%^%astro/%')"
archiverslist="$(echo "$archiverslist" | sed 's^ ^ archivers/^g' | sed 's%^%archivers%')"
audiolist="$(echo "$audiolist" | sed 's^ ^ audio/^g' | sed 's%^%audio%')"
cadlist="$(echo "$cadlist" | sed 's^ ^ cad/^g' | sed 's%^%cad%')"
chineselist="$(echo "$chineselist" | sed 's^ ^ chinese/^g' | sed 's%^%chinese%')"
commslist="$(echo "$commslist" | sed 's^ ^ comms/^g' | sed 's%^%comms%')"
converterslist="$(echo "$converterslist" | sed 's^ ^ converters/^g' | sed 's%^%converters%')"
databaseslist="$(echo "$databaseslist" | sed 's^ ^ databases/^g' | sed 's%^%databases%')"
devellist="$(echo "$devellist" | sed 's^ ^ devel/^g' | sed 's%^%devel%')"
editorslist="$(echo "$editorslist" | sed 's^ ^ editors/^g' | sed 's%^%editors%')"
gameslist="$(echo "$gameslist" | sed 's^ ^ games/^g' | sed 's%^%games%')"
graphicslist="$(echo "$graphicslist" | sed 's^ ^ graphics/^g' | sed 's%^%graphics%')"
geolist="$(echo "$geolist" | sed 's^ ^ geo/^g' | sed 's%^%geo%')"
langlist="$(echo "$langlist" | sed 's^ ^ lang/^g' | sed 's%^%lang%')"
maillist="$(echo "$maillist" | sed 's^ ^ mail/^g' | sed 's%^%mail%)"
mathllist="$(echo "$mathlist" | sed 's^ ^ math/^g' | sed 's%^%%)"
